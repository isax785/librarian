...
The `Librarian` (aka **Library Updater**) is an agent to ease the syncronization of files from an external folder to a local one: when the first copy is realized, the sync of the .two folders is maintained by updating the local folder by applying only the variations respect to the new version of the external one.

![](./doc/Librarian.excalidraw.png)

> Documentation [here](./doc/doc_librarian.md).
